...
def open_web(link, interval):
    cmd("am start -a android.intent.action.VIEW -d \"https://%s\"" % link, interval)
    return True


def flush_web():
    cmd("pm clear com.android.chrome", interval=1)
    cmd("am start -a android.intent.action.VIEW -d \"https://naver.com\"", interval=1)
